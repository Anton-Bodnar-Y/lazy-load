...
<script>
    export default {
        mounted() {
            this.getArticles(this.defaultOffset, this.defaultLimit);
        },
		...
		methods: {
			getArticles: function (defaultOffset, defaultLimit) {
			...
			addArticles: function (defaultOffset, defaultLimit) {
				axios.get('/public/get-article/' + defaultOffset + '/' + defaultLimit).then(response => {
					
					this.countRowTotal = response.data[0];
					this.countRowThis = response.data[1].length;
					
					var contentAddToList = '';
					for (var i = 0; i < response.data[1].length; i++) {
						contentAddToList += '<li class="art-block" v-for="result in results">' + response.data[1][i].title + '</li>';
					}
					
					document.getElementById('containerList').innerHTML += contentAddToList;
			...
			scrollFunction: function() {
				...
				if ((windowHeight > (positionElement+30)) && (this.current === true) && (this.countRowThis < this.countRowTotal)) {
					...
					this.addArticles(this.defaultOffset, this.defaultLimit);
				}
			},
		},
    }
